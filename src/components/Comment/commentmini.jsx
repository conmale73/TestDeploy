...
function Commentmini({comment,replies,handleReply}) {
    return ( 
        <div>
            { (
                ...
                <div>
                    ...
                    {comment?.creator?.admin && (<b className="ml-4 bg-yellow-400 rounded px-2">Quản trị viên</b>)}
                </div>
                <p>{comment.content}</p>
         ...
         replies.length > 0 && replies?.map(item=>(
            <div className="admin bg-gray-100 p-4 my-4 border border-gray-200 relative">
                <Commentmini replies={[]} comment={item} key={item.id} handleReply={handleReply}></Commentmini>
            </div>
         ))
        }
